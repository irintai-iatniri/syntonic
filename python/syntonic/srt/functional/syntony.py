...
class SyntonyFunctional:
    ...
    def _enumerate_by_norm(self, norm_sq: int) -> List[WindingState]:
        ...
        states = []
        max_val = int(math.sqrt(norm_sq)) + 1
        for n7 in range(-max_val, max_val + 1):
            for n8 in range(-max_val, max_val + 1):
                for n9 in range(-max_val, max_val + 1):
                    for n10 in range(-max_val, max_val + 1):
                        if n7*n7 + n8*n8 + n9*n9 + n10*n10 == norm_sq:
                            states.append(winding_state(n7, n8, n9, n10))
        return states
