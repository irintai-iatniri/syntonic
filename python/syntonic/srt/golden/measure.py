...
import math
from typing import Dict, Optional, Callable, Iterator, TYPE_CHECKING
...
class GoldenMeasure:
    ...
    def theta_sum(
        ...
        total = 0.0
        for n in enumerate_windings(max_norm):
            # Combined weight: exp(-|n|^2/phi) * exp(-pi*|n|^2/t)
            # = exp(-|n|^2 * (1/phi + pi/t))
            combined_factor = n.norm_squared * (1.0 / self._phi + math.pi / t)
            total += math.exp(-combined_factor)
        return total
...
def compute_partition_function(max_norm: int = 10, phi: Optional[float] = None) -> float:
    ...
    Returns:
        Partition function value
    """
    measure = GoldenMeasure(phi=phi)
    return measure.partition_function(max_norm)
