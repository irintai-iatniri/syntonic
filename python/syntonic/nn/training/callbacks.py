"""
Training Callbacks for Syntonic Networks.

Callbacks for monitoring syntony, detecting archonic patterns,
and controlling training flow.

Source: CRT.md §12.2
"""

from __future__ import annotations
from typing import Optional, Dict, Any, List, TYPE_CHECKING
import math
import json
from pathlib import Path

if TYPE_CHECKING:
    from syntonic.nn.training.trainer import SyntonicTrainer

PHI = (1 + math.sqrt(5)) / 2
Q_DEFICIT = 0.027395146920
S_TARGET = PHI - Q_DEFICIT


class Callback:
    """Base class for training callbacks."""

    def on_train_begin(self, trainer: 'SyntonicTrainer'):
        """Called at start of training."""
        pass

    def on_train_end(self, trainer: 'SyntonicTrainer'):
        """Called at end of training."""
        pass

    def on_epoch_begin(self, trainer: 'SyntonicTrainer', epoch: int):
        """Called at start of each epoch."""
        pass

    def on_epoch_end(
        self,
        trainer: 'SyntonicTrainer',
        epoch: int,
        train_metrics: Dict[str, float],
        val_metrics: Optional[Dict[str, float]],
    ):
        """Called at end of each epoch."""
        pass

    def on_batch_begin(self, trainer: 'SyntonicTrainer', batch_idx: int):
        """Called at start of each batch."""
        pass

    def on_batch_end(
        self,
        trainer: 'SyntonicTrainer',
        batch_idx: int,
        metrics: Dict[str, Any],
    ):
        """Called at end of each batch."""
        pass


class SyntonyCallback(Callback):
    """
    Monitor and log syntony during training.

    Tracks syntony evolution and provides statistics.

    Example:
        >>> callback = SyntonyCallback(log_interval=10)
        >>> trainer = SyntonicTrainer(model, loader, callbacks=[callback])
    """

    def __init__(
        self,
        log_interval: int = 10,
        target_syntony: Optional[float] = None,
        verbose: bool = True,
    ):
        """
        Initialize syntony callback.

        Args:
            log_interval: Epochs between logs
            target_syntony: Target syntony (default: φ - q)
            verbose: Print updates
        """
        self.log_interval = log_interval
        self.target_syntony = target_syntony if target_syntony is not None else S_TARGET
        self.verbose = verbose

        self.history: List[Dict[str, float]] = []
        self._best_syntony = 0.0
        self._best_epoch = 0

    def on_epoch_end(
        self,
        trainer: 'SyntonicTrainer',
        epoch: int,
        train_metrics: Dict[str, float],
        val_metrics: Optional[Dict[str, float]],
    ):
        """Record syntony at end of epoch."""
        syntony = train_metrics.get('syntony', 0.0)

        record = {
            'epoch': epoch,
            'train_syntony': syntony,
            'train_loss': train_metrics.get('loss', 0.0),
        }

        if val_metrics:
            record['val_syntony'] = val_metrics.get('syntony', 0.0)
            record['val_loss'] = val_metrics.get('loss', 0.0)

        self.history.append(record)

        # Track best
        if syntony > self._best_syntony:
            self._best_syntony = syntony
            self._best_epoch = epoch

        # Log
        if self.verbose and (epoch + 1) % self.log_interval == 0:
            gap = self.target_syntony - syntony
            status = "↑" if syntony > self.target_syntony - 0.1 else "↓"
            print(f"Epoch {epoch+1}: S={syntony:.4f} (target gap: {gap:.4f}) {status}")

    def on_train_end(self, trainer: 'SyntonicTrainer'):
        """Print summary at end of training."""
        if self.verbose and self.history:
            final = self.history[-1]
            print(f"\nTraining complete:")
            print(f"  Final syntony: {final['train_syntony']:.4f}")
            print(f"  Best syntony: {self._best_syntony:.4f} (epoch {self._best_epoch + 1})")
            print(f"  Target: {self.target_syntony:.4f}")

    @property
    def best_syntony(self) -> float:
        """Get best syntony achieved."""
        return self._best_syntony

    @property
    def syntony_trend(self) -> float:
        """Get recent syntony trend."""
        if len(self.history) < 10:
            return 0.0

        recent = [h['train_syntony'] for h in self.history[-10:]]
        mid = len(recent) // 2
        return sum(recent[mid:]) / len(recent[mid:]) - sum(recent[:mid]) / len(recent[:mid])


class ArchonicEarlyStop(Callback):
    """
    Stop training when archonic pattern is detected.

    Detects when the network is cycling without syntony improvement
    and triggers early stopping or escape mechanisms.

    Example:
        >>> callback = ArchonicEarlyStop(patience=20)
        >>> trainer = SyntonicTrainer(model, loader, callbacks=[callback])
    """

    def __init__(
        self,
        patience: int = 20,
        variance_threshold: float = 0.01,
        min_improvement: float = 0.001,
        escape_instead: bool = False,
        verbose: bool = True,
    ):
        """
        Initialize archonic early stop.

        Args:
            patience: Epochs of no improvement before stopping
            variance_threshold: Threshold for detecting cycling
            min_improvement: Minimum syntony improvement required
            escape_instead: Inject noise instead of stopping
            verbose: Print warnings
        """
        self.patience = patience
        self.variance_threshold = variance_threshold
        self.min_improvement = min_improvement
        self.escape_instead = escape_instead
        self.verbose = verbose

        self._syntony_history: List[float] = []
        self._no_improvement_count = 0
        self._best_syntony = 0.0
        self._should_stop = False
        self._archonic_count = 0

    def on_epoch_end(
        self,
        trainer: 'SyntonicTrainer',
        epoch: int,
        train_metrics: Dict[str, float],
        val_metrics: Optional[Dict[str, float]],
    ):
        """Check for archonic pattern at end of epoch."""
        syntony = train_metrics.get('syntony', 0.0)
        self._syntony_history.append(syntony)

        # Check improvement
        if syntony > self._best_syntony + self.min_improvement:
            self._best_syntony = syntony
            self._no_improvement_count = 0
        else:
            self._no_improvement_count += 1

        # Check for archonic pattern
        is_archonic = self._detect_archonic()

        if is_archonic:
            self._archonic_count += 1

            if self.verbose:
                print(f"⚠ Archonic pattern detected at epoch {epoch + 1}")

            if self.escape_instead:
                self._trigger_escape(trainer)
            elif self._no_improvement_count >= self.patience:
                self._should_stop = True
                if self.verbose:
                    print(f"⛔ Early stopping: archonic pattern for {self.patience} epochs")

    def _detect_archonic(self) -> bool:
        """Detect archonic cycling pattern."""
        if len(self._syntony_history) < 20:
            return False

        recent = self._syntony_history[-20:]
        mean_S = sum(recent) / len(recent)
        var_S = sum((s - mean_S) ** 2 for s in recent) / len(recent)

        # Check for high variance (cycling)
        if var_S < self.variance_threshold:
            return False

        # Check for no trend (stuck)
        mid = len(recent) // 2
        trend = sum(recent[mid:]) / len(recent[mid:]) - sum(recent[:mid]) / len(recent[:mid])

        return abs(trend) < self.min_improvement

    def _trigger_escape(self, trainer: 'SyntonicTrainer'):
        """Inject noise to escape archonic pattern."""
        for param in trainer.model.parameters():
            if param.data is not None:
                noise_scale = 0.01 * param.data.std()
                param.data.add_(torch.randn_like(param.data) * noise_scale)

    @property
    def should_stop(self) -> bool:
        """Check if training should stop."""
        return self._should_stop

    @property
    def archonic_count(self) -> int:
        """Number of archonic detections."""
        return self._archonic_count


class SyntonyCheckpoint(Callback):
    """
    Save checkpoints based on syntony.

    Saves model when syntony improves or at regular intervals.

    Example:
        >>> callback = SyntonyCheckpoint('checkpoints/', save_best=True)
        >>> trainer = SyntonicTrainer(model, loader, callbacks=[callback])
    """

    def __init__(
        self,
        save_dir: str,
        save_best: bool = True,
        save_interval: int = 10,
        min_syntony: float = 0.0,
    ):
        """
        Initialize checkpoint callback.

        Args:
            save_dir: Directory for checkpoints
            save_best: Save on syntony improvement
            save_interval: Epochs between regular saves
            min_syntony: Minimum syntony to trigger save
        """
        self.save_dir = Path(save_dir)
        self.save_dir.mkdir(parents=True, exist_ok=True)
        self.save_best = save_best
        self.save_interval = save_interval
        self.min_syntony = min_syntony

        self._best_syntony = 0.0

    def on_epoch_end(
        self,
        trainer: 'SyntonicTrainer',
        epoch: int,
        train_metrics: Dict[str, float],
        val_metrics: Optional[Dict[str, float]],
    ):
        """Save checkpoint if criteria met."""
        import torch
        syntony = train_metrics.get('syntony', 0.0)

        # Save best
        if self.save_best and syntony > self._best_syntony and syntony > self.min_syntony:
            self._best_syntony = syntony
            path = self.save_dir / 'best_model.pt'
            torch.save({
                'epoch': epoch,
                'model_state_dict': trainer.model.state_dict(),
                'optimizer_state_dict': trainer.optimizer.state_dict(),
                'syntony': syntony,
                'loss': train_metrics.get('loss', 0.0),
            }, path)

        # Regular interval save
        if (epoch + 1) % self.save_interval == 0:
            path = self.save_dir / f'checkpoint_epoch_{epoch+1}.pt'
            torch.save({
                'epoch': epoch,
                'model_state_dict': trainer.model.state_dict(),
                'optimizer_state_dict': trainer.optimizer.state_dict(),
                'syntony': syntony,
                'loss': train_metrics.get('loss', 0.0),
            }, path)


class MetricsLogger(Callback):
    """
    Log training metrics to file.

    Saves detailed metrics in JSON format for analysis.

    Example:
        >>> callback = MetricsLogger('logs/training_metrics.json')
        >>> trainer = SyntonicTrainer(model, loader, callbacks=[callback])
    """

    def __init__(
        self,
        log_path: str,
        include_gradients: bool = False,
        include_weights: bool = False,
    ):
        """
        Initialize metrics logger.

        Args:
            log_path: Path to log file
            include_gradients: Log gradient statistics
            include_weights: Log weight statistics
        """
        self.log_path = Path(log_path)
        self.log_path.parent.mkdir(parents=True, exist_ok=True)
        self.include_gradients = include_gradients
        self.include_weights = include_weights

        self.logs: List[Dict[str, Any]] = []

    def on_epoch_end(
        self,
        trainer: 'SyntonicTrainer',
        epoch: int,
        train_metrics: Dict[str, float],
        val_metrics: Optional[Dict[str, float]],
    ):
        """Log metrics at end of epoch."""
        import torch
        log_entry = {
            'epoch': epoch,
            'train': train_metrics,
            'val': val_metrics,
            'lr': trainer.optimizer.param_groups[0]['lr'],
        }

        # Gradient statistics
        if self.include_gradients:
            grad_norms = []
            for param in trainer.model.parameters():
                if param.grad is not None:
                    grad_norms.append(param.grad.norm().item())
            if grad_norms:
                log_entry['grad_norm_mean'] = sum(grad_norms) / len(grad_norms)
                log_entry['grad_norm_max'] = max(grad_norms)

        # Weight statistics
        if self.include_weights:
            weight_norms = []
            for param in trainer.model.parameters():
                weight_norms.append(param.norm().item())
            if weight_norms:
                log_entry['weight_norm_mean'] = sum(weight_norms) / len(weight_norms)
                log_entry['weight_norm_max'] = max(weight_norms)

        self.logs.append(log_entry)

    def on_train_end(self, trainer: 'SyntonicTrainer'):
        """Save logs to file."""
        with open(self.log_path, 'w') as f:
            json.dump(self.logs, f, indent=2)

    def get_dataframe(self):
        """Convert logs to pandas DataFrame."""
        try:
            import pandas as pd
            return pd.DataFrame(self.logs)
        except ImportError:
            return None
