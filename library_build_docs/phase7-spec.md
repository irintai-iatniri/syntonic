...
**Key Results from Theory (CRT.md §12.1-12.2):**
- 35% faster convergence in high-S networks
- Convergence rate: λ ~ ln(1/(1-S_avg)) ≈ 2.21 at S ≈ 0.889
...
class ArchonicDetector:
    ...
    def _estimate_basin_volume(
        ...
        Derivation (from Breaking_Free.md §53):
        - q = 0.027395 (syntony deficit)
        - φ = 1.618034 (golden ratio)
        - q/φ ≈ 0.0169
        - exp(-0.0169) ≈ 0.9832
        - V_B = 1 - 0.9832 ≈ 0.0168 ≈ 1.7%
        
        Physical meaning: ~1.7% of random states will be captured
        by a typical zero-sum Archonic trap.
        
        Adjusted by local syntony deficit for actual network state.
        """
...
| Equation | Expression | Source | Derivation |
|----------|------------|--------|------------|
| **D-Layer** | x → x + ReLU(W_D·x + b_D) | CRT.md §12.2 | ReLU for complexity expansion |
| **H-Layer** | x → x - σ(W_H·x) + tanh(W_S·x) | CRT.md §12.2 | σ damps, tanh stabilizes |
| **R-Block** | R(x) = H(D(x)) | CRT.md §12.2 | Complete DHSR cycle |
| **S_model** | 1 - \|D(x)-x\| / \|D(x)-H(D(x))\| | CRT.md §12.2 | Unified syntony form |
| **Syntonic Loss** | L = L_task + λ(1-S) + μC_{iπ} | CRT.md §12.2 | Task + syntony + phase |
| **Phase Alignment** | C_{iπ} = \|Arg Tr[e^{iπρ}] - π/2\|² | CRT.md §12.2 | i ≃ π constraint |
| **Convergence Rate** | λ ~ ln(1/(1-S_avg)) ≈ 2.21 | CRT.md §12.1 | At S_avg = 0.889 |
| **Chaos Reduction** | λ_S = λ_max(1 - ηS) ≈ 0.012 | CRT.md §12.1 | High-S regime |
| **LR Modulation** | lr_eff = lr × (1 + α(S - S_target)) | CRT.md §12.2 | Boost in coherent regime |
| **Archonic Condition** | R̂^n\|ψ⟩ = \|ψ⟩, S < φ - q | Breaking_Free.md §4 | Fixed point + low syntony |
| **Basin Volume** | V_B = 1 - e^{-q/φ} ≈ 1.7% | Breaking_Free.md §53 | Golden measure integral |
| **Escape Energy** | ε > 1/φ² ≈ 0.382 | Breaking_Free.md §55 | Phase transition threshold |
| **Syntony Target** | S_target = φ - q ≈ 1.591 | CRT.md §4.1 | Vacuum syntony bound |
